...
export async function GET(request) {
  ...
  const [total, rows, agg] = await prisma.$transaction([
    prisma.adSpentShopee.count({ where }),
    prisma.adSpentShopee.findMany({
      where,
      orderBy: { date: 'desc' },
      skip: (page - 1) * limit,
      take: limit,
    }),
    prisma.adSpentShopee.aggregate({
      where,
      _sum: { spent: true, clicks: true, orders: true, revenue: true, impressions: true },
    }),
  ])
