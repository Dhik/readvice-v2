...
export default function ShopeeChartPanel({ dateFrom, dateTo, metrics }) {
  ...
  useEffect(() => {
    const qs = new URLSearchParams({ dateFrom: dateFrom || '', dateTo: dateTo || '' })
    fetch(`/api/affiliate/shopee/gmv-by-channel?${qs}`)
      .then(r => r.json())
      .then(d => {
        const data = d.data ?? []
        if (!donutChart.current) return
        donutChart.current.data.labels                 = data.map(x => x.channel)
        donutChart.current.data.datasets[0].data       = data.map(x => x.gmv)
      ...
      <div className="sv-panel-body p-0">
        <div className="flex flex-col h-full overflow-y-auto">
          ...
          <div className="p-2 border-b border-cream" style={{ flex: '0 0 140px', minHeight: 0 }}>
            <div className="text-[9px] font-semibold text-dark2 uppercase mb-1">GMV by Channel</div>
          ...
          <div className="p-1.5 border-b border-cream flex-shrink-0">
            <div className="metric-row">
              <div className="metric-card">
                <div className="metric-card-label">Affiliates</div>
                <div className="metric-card-value">{fmtNum(metrics?.total_affiliates)}</div>
              </div>
              <div className="metric-card">
                <div className="metric-card-label">Orders</div>
                <div className="metric-card-value">{fmtNum(metrics?.total_orders)}</div>
              </div>
            </div>
            <div className="metric-row">
              <div className="metric-card">
                <div className="metric-card-label">Avg ROI</div>
                <div className="metric-card-value">{fmtPct(metrics?.avg_roi)}</div>
              </div>
              <div className="metric-card">
                <div className="metric-card-label">New Buyer%</div>
                <div className="metric-card-value">{fmtPct(metrics?.new_buyer_rate)}</div>
              </div>
            </div>
          </div>
