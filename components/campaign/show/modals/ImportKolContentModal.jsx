...
export default function ImportKolContentModal({ isOpen, onClose, campaignId, onSuccess }) {
  ...
  async function handleImport() {
    ...
    try {
      ...
      fd.append('file', file)
      const res = await fetch(`/api/campaign-import/kol-content/${campaignId}`, {
        method: 'POST',
        body: fd,
      })
      ...
      setResult(data)
      if (data.imported > 0) {
        toast.success(`${data.imported} KOL rows imported`)
        onSuccess?.()
      }
    } catch {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center"
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-xl mx-4"
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-cream">
          <h3 className="font-semibold text-dark1 text-sm">
            <i className="fas fa-star text-orange mr-1"></i> Import KOL Content
          </h3>
        ...
        <div className="p-4 space-y-3">
          <div className="text-xs text-dark2">
            ...
            KOL columns: username, creator_name, pic, task_name, channel, link, product, kode_ads, rate_card,
            <strong> upload_date</strong>, dealing_upload_date, <strong>kol_followers</strong>
          </div>
          <div>
        ...
        <div className="flex justify-end gap-2 px-4 py-3 border-t border-cream">
          ...
          <button onClick={handleImport} disabled={loading || !file}
            className="sv-act-btn sv-act-outline-r text-xs flex items-center gap-1">
