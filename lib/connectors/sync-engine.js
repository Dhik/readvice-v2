...
import { fetchDriveFile } from './sources/drive'
import { resolveSourceConfig } from './source-config'
import { applyTransform, validateColumnMapping, TRANSFORMS } from './transforms'

// ── Fetch dispatch by sourceType (Part E) ────────────────────────────────────
// Selects the row-fetcher by the connector's sourceType and feeds it the EFFECTIVE
// sourceConfig (back-compat: google_sheets with null sourceConfig reads the legacy
// columns). Returns string[][] in getSheetRows' shape — the transform/write layer is
// reused unchanged. `opts.fetchRows` / `opts.fetchDriveFile` are injectable test seams.
async function fetchRowsForConnector(connector, opts = {}) {
  const sourceType = connector.sourceType || 'google_sheets'
  const cfg = resolveSourceConfig(connector)
  if (sourceType === 'google_sheets') {
    const fetch = opts.fetchRows ?? getSheetRows           // unchanged signature (spreadsheetId, range)
    return fetch(cfg.spreadsheetId, `${cfg.sheetTab}!${cfg.dataRange}`)
  }
  if (sourceType === 'google_drive_file') {
    const fetch = opts.fetchDriveFile ?? fetchDriveFile     // (sourceConfig) → string[][]
    return fetch(cfg)
  }
  if (sourceType === 'onedrive_file') {
    throw new Error('OneDrive connectors are not yet supported (pending OAuth design — E3).')
  }
  throw new Error(`Unknown connector sourceType '${sourceType}'`)
}
...
/**
 * Run a connector's sync. Dispatches the FETCH by sourceType (Part E) and the WRITE by
 * connectorType/targetTable (unchanged).
 * @param {number} connectorId
 * @param {object} [opts]
 * @param {Function} [opts.fetchRows]     google_sheets fetcher (spreadsheetId, range) → string[][] (test seam).
 * @param {Function} [opts.fetchDriveFile] google_drive_file fetcher (sourceConfig) → string[][] (test seam).
 */
export async function syncConnector(connectorId, opts = {}) {
  ...
  // Source-agnostic: dispatched by sourceType, fed the effective sourceConfig.
  const rows = await fetchRowsForConnector(connector, opts)
