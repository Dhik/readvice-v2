...
// Mirrors the engine's per-CELL `dummy` flag (source === 'DUMMY') under the
// "becomes real with time" posture: `cohortSize` (period-0 acquisition count) is
// the REAL anchor (the 1/36 real cell) and `periodIndex` is a real structural
// index → dummy:false. The retention curve itself — `retentionPct` /
// `customersRetained` (period 1+) — is fabricated decay today → dummy:true (turns
// real automatically as months elapse; no backfill).
export const FIELD_MANIFEST = [
  { key: 'cohortSize',        label: 'Cohort size (period-0)',     unit: 'count', dummy: false, source: 'REAL-DERIVED' },
  { key: 'periodIndex',       label: 'Period index (months since)',unit: 'count', dummy: false, source: 'REAL' },
  { key: 'retentionPct',      label: 'Retention %',                unit: '%',     dummy: true,  source: 'DUMMY' },
  { key: 'customersRetained', label: 'Customers retained',         unit: 'count', dummy: true,  source: 'DUMMY' },
]
