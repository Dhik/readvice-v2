...
// 100% REAL, expense-only — every engine response carries `dummy: false`, so all
// params are dummy:false. NOTE: there is deliberately NO ROAS / revenue param
// (no spend→revenue attribution exists yet — Wave 3), so calc fields here can only
// combine spend/share figures, never an efficiency-vs-sales ratio.
export const FIELD_MANIFEST = [
  { key: 'spend',         label: 'Spend',            unit: 'IDR', dummy: false, source: 'REAL' },
  { key: 'sharePct',      label: 'Share of spend',   unit: '%',   dummy: false, source: 'REAL-DERIVED' },
  { key: 'cumulativePct', label: 'Cumulative share', unit: '%',   dummy: false, source: 'REAL-DERIVED' },
  { key: 'changePct',     label: 'MoM change',       unit: '%',   dummy: false, source: 'REAL-DERIVED' },
]
