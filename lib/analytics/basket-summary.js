...
// 100% REAL co-purchase math on real OrderItem counts → every response carries
// `dummy: false`, so all params are dummy:false. The honesty signal here is SAMPLE
// SIZE, not fabrication: the engine's `smallSample` flag (multi-item orders < 50)
// is the caveat — noted in `source` for the lift ratio (n=1 lifts are unstable),
// but no field is dummy.
export const FIELD_MANIFEST = [
  { key: 'cooccur',        label: 'Co-occurrence count', unit: 'count', dummy: false, source: 'REAL' },
  { key: 'support',        label: 'Support',             unit: 'ratio', dummy: false, source: 'REAL-DERIVED' },
  { key: 'supportPct',     label: 'Support %',           unit: '%',     dummy: false, source: 'REAL-DERIVED' },
  { key: 'confidenceAtoB', label: 'Confidence A→B',      unit: '%',     dummy: false, source: 'REAL-DERIVED' },
  { key: 'confidenceBtoA', label: 'Confidence B→A',      unit: '%',     dummy: false, source: 'REAL-DERIVED' },
  { key: 'lift',           label: 'Lift',                unit: 'x',     dummy: false, source: 'REAL-DERIVED (small-sample)' },
  { key: 'orders',         label: 'Orders containing SKU', unit: 'count', dummy: false, source: 'REAL' },
  { key: 'revenue',        label: 'SKU revenue',         unit: 'IDR',   dummy: false, source: 'REAL' },
]
